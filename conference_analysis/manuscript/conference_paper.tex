...
The model--dataset pair is the experimental cell, and the applicant query is the independent observational unit. Counterfactuals are nested within queries; they are not treated as independent observations. The primary estimand is query coverage. Secondary estimands are query-level mean sparsity, proximity, and within-set diversity. For a covered query, sparsity and proximity are first averaged over its valid counterfactuals, preventing queries that return more alternatives from receiving greater statistical weight.

Two-sided 95\% Wilson score intervals quantify uncertainty in coverage. For continuous counterfactual metrics, two-sided 95\% percentile intervals are obtained from 10,000 nonparametric bootstrap resamples of queries, with seed 2026. Intervals are calculated separately within every model--dataset cell. The analysis is exploratory: the ten cases were chosen as a computational stress test rather than by a prospective power calculation, and each classifier selected its own highest-risk cohort. Therefore, we do not perform paired hypothesis tests or claim statistically significant superiority between classifiers. Cross-model differences are reported as descriptive effect contrasts with uncertainty. Confirmatory testing requires a shared applicant cohort and repeated DiCE seeds. No correction for multiple comparisons is applied because no family of null-hypothesis tests is conducted.

\begin{table*}[t]
\caption{Statistical Design and Inferential Scope}
\label{tab:statistical-design}
\centering\scriptsize
\begin{tabular}{p{3.0cm}p{12.5cm}}
\toprule
Design element & Specification \\
\midrule
Experimental cell & One classifier--dataset combination; 15 cells in total (five classifiers across three datasets). \\
Independent unit & Applicant query. Generated counterfactuals are nested within queries and are not counted as independent observations. \\
Query sample & Ten held-out applicants with the highest model-specific predicted default probabilities in each cell; stress-test sample rather than a probability sample. \\
Primary estimand & Query coverage: proportion of the ten queries for which at least one independently verified valid counterfactual is generated. \\
Secondary estimands & Query-level mean number of changed features, query-level mean mixed-type proximity, and within-query mean pairwise diversity. \\
Aggregation rule & Sparsity and proximity are first averaged across valid counterfactuals within each query, then summarized across queries to prevent unequal weighting by generation yield. \\
Coverage uncertainty & Two-sided 95\% Wilson score confidence interval calculated separately for each model--dataset cell. \\
Continuous uncertainty & Two-sided 95\% nonparametric percentile bootstrap interval based on 10,000 query-level resamples with replacement; fixed seed 2026. \\
Cross-model comparison & Descriptive effect contrasts only. Model-specific query cohorts are not paired, so no paired hypothesis tests or claims of statistically significant model superiority are made. \\
Multiplicity & No adjustment applied because the study does not conduct a family of null-hypothesis significance tests. \\
Inferential scope & Exploratory and hypothesis-generating. Confirmatory inference requires a larger shared query cohort, repeated DiCE seeds, and a prespecified power analysis. \\
\bottomrule
\end{tabular}
\end{table*}
...
Uncertainty is substantial at $n=10$ queries per cell. Full coverage corresponds to a Wilson 95\% interval of [0.722, 1.000], 90\% coverage to [0.596, 0.982], and the 50\% logistic-regression coverage on Give Me Some Credit to [0.237, 0.763]. Query-level bootstrap analysis nevertheless preserves the main descriptive pattern. For German Credit, mean changes were 4.28 [3.80, 4.73] for logistic regression and 2.70 [2.50, 2.85] for the decision tree. For Give Me Some Credit, logistic regression required 5.55 [4.55, 6.90] changes among covered queries, compared with 4.01 [3.14, 4.86] for the tree. Brackets report 95\% intervals. These wide coverage intervals reinforce that the current experiment supports hypothesis generation rather than definitive model ranking; complete cell-level intervals are provided in the repository.
